<script>
	...
	import { isVisible } from '$lib/state.svelte';

	const shouldAnimate = !isVisible.heroCopy;
	let svgEl = $state();
	let sectionEl = $state();
...
<section
	bind:this={sectionEl}
	class="relative flex h-screen items-center justify-center bg-background"
>
	<svg bind:this={svgEl} class="absolute inset-0 h-full w-full"></svg>

	<div class="pointer-events-none relative z-10 w-full max-w-7xl p-8 transition-opacity">
		<div
			class="w-fit bg-muted/60 p-8 backdrop-blur-xs {isVisible.heroCopy
