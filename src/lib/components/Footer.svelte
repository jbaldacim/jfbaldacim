...
<footer class="w-full border-t bg-card text-muted-foreground transition-colors duration-300">
	...
	>
		...
		<div class="flex flex-col items-center gap-1 md:items-start">
			<a
				href={resolve('/')}
				class="font-bold tracking-wide text-foreground uppercase transition-colors duration-300 hover:text-primary"
			>
				João Baldacim
			</a>
			<p class="text-xs">
		...
		<!-- Internal links -->
		<ul class="flex flex-wrap items-center justify-center gap-6 text-sm tracking-wider">
			{#each internalLinks as link (link.label)}
				{#if link.path}
					<li>
						<a href={resolve(link.path)} class="transition-colors duration-200 hover:text-primary">
							{link.label}
						</a>
					</li>
				{/if}
