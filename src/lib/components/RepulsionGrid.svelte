...
<script lang="ts">
	...
	let actualMode = $derived(
		renderMode !== 'auto' ? renderMode : circleCount > autoThreshold ? 'canvas' : 'svg'
	);
			...
			for (let xi = 0; xi < cols; xi++) {
				...
				for (let sy = 0; sy < res; sy++) {
					for (let sx = 0; sx < res; sx++) {
						const x = offsetX + xi * cellSize + sx * subCellSize + subCellSize / 2;
						const y = offsetY + yi * cellSize + sy * subCellSize + subCellSize / 2;
	...
	$effect(() => {
		if (actualMode === 'canvas' && canvasEl && containerWidth && containerHeight) {
...
<div class={cn('flex flex-col items-center gap-2', className)} {style}>
	...
	{#if title}
		<span class="text-center font-light tracking-wider">
			{title}
		</span>
	{/if}
</div>
